...
use App\Lib\IDatabase;
use App\Lib\IDbRecord;
use App\Lib\Log;
...
class MySQLi implements IDatabase
{
    ...
    protected function tryReconnect($call, $params)
    {
        ...
        for ($i = 0; $i < 2; $i++)
        {
            ...
            if ($result === false)
            {
                ...
                {
                    Log::write(__CLASS__." SQL ERROR".$this->mysqli->error);
                    return false;
                }
            }
            break;
    ...
    function query($sql, $resultmode = null)
    {
        ...
        if (!$result)
        {
            throw new MysqlException(__CLASS__ . " SQL Error:" . $this->errorMessage($sql));
        }
        if (is_bool($result))
        {
            return $result;
